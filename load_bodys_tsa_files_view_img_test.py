            ...
            if zone_body_threat.size > 0:
                ...
                for zone_threat in zone_body_threat:
                    ...
                    if zone_threat[0] <= count_z + z_min <= zone_threat[1]:
                        if zone_threat[2] == i:
                            path = 'data/test/data_zones_body/zone_threat' \
                                   + str(i) + '/' + loaded_body + '_z_' + str(count_z + z_min) + '.png'
                            if zone_threat[2] != flip_zones[i - 1][1] and zone_threat[2] != not_flip_zones[0] and \
                                ...
                                path_flip = 'data/test/data_zones_body/zone_threat' \
                                    + str(flip_zones[i - 1][1]) \
                                    + '/' + loaded_body + '_z_' + str(
                                        count_z + z_min) + '.png'
                        else:
                            ...
                            if zone_threat[2] == border_zone_up1 or \
                                ...
                                break
                if path.__eq__(''):
                    path = 'data/test/data_zones_body/zone' \
                           + str(i) + '/' + loaded_body + '_z_' + str(count_z + z_min) + '.png'
